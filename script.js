...
async function reloadProducts(){products=(await fetchProducts()).sort((a,b)=>a.id-b.id);render(activeFilter);renderFeatured();buildCombo();renderAdmin()}
...
function renderFeatured(){
 const section=$('#destacado');if(!section)return;
 const featured=products.find(p=>p.featured&&p.visible!==false)||products.find(p=>p.visible!==false);
 if(!featured){section.hidden=true;return}
 section.hidden=false;
 const image=section.querySelector('.spotlight-image img'),title=section.querySelector('.spotlight-copy h2'),description=section.querySelector('.spotlight-copy p'),price=section.querySelector('.spotlight-price strong'),priceNote=section.querySelector('.spotlight-price small'),link=section.querySelector('.spotlight-copy .button');
 if(image){image.src=featured.image;image.alt=`${featured.name} destacado`}
 if(title)title.textContent=featured.name;
 if(description)description.textContent=featured.description||'Consultá disponibilidad y opciones de este producto destacado.';
 if(price)price.textContent=money(featured.price);
 if(priceNote)priceNote.textContent=statusLabels[featured.status]||'Disponible';
 if(link){const unavailable=featured.status==='out';link.href=unavailable?'#':whatsappUrl(featured.name);link.textContent=unavailable?'Sin stock por el momento':'Consultar este producto';link.classList.toggle('disabled',unavailable);link.setAttribute('aria-disabled',unavailable?'true':'false');if(unavailable){link.removeAttribute('target');link.removeAttribute('rel')}else{link.target='_blank';link.rel='noopener'}}
}

async function keepOnlyFeatured(productId){
 if(!productId)return;
 if(!cloudReady){for(const other of products){if(other.id!==productId&&other.featured)await localPut({...other,featured:false})}return}
 const {error}=await cloud.from('products').update({featured:false}).eq('featured',true).neq('id',productId);if(error)throw error;
}
...
$('#productForm').addEventListener('submit',async e=>{e.preventDefault();if(!requireAdmin()||busy)return;busy=true;const submit=e.submitter;submit.disabled=true;submit.textContent='Guardando…';try{const oldId=Number($('#editId').value)||undefined;let image=$('#editImage').value.trim();const upload=$('#editUpload').files[0];if(upload)image=await uploadImage(upload);const p={...(oldId?products.find(x=>x.id===oldId):{}),name:$('#editName').value.trim(),category:$('#editCategory').value,label:$('#editLabel').value.trim(),price:Number($('#editPrice').value),stock:Number($('#editStock').value),status:$('#editStatus').value,description:$('#editDescription').value.trim(),image,visible:$('#editVisible').checked,featured:$('#editFeatured').checked};if(oldId)p.id=oldId;const savedId=await saveProduct(p);if(p.featured)await keepOnlyFeatured(Number(savedId||oldId));editorModal.close();await reloadProducts()}catch(err){console.error(err);alert(`No se pudo guardar el producto: ${err.message}`)}finally{busy=false;submit.disabled=false;submit.textContent='Guardar cambios'}});
